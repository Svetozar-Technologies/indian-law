...
| --- | --- | --- |
| R1 | Make a first prototype of an active-law list republished as GitHub Pages. | `docs/index.html`, language indexes, and generated law pages are produced by `scripts/build-site.mjs`. |
...
| R3 | Support official languages such as English, Hindi, Tamil, Telugu, Bengali, Kannada, and Marathi when available. | `data/languages.json` configures these languages; `scripts/discover-regional-sources.mjs` records regional official PDF availability. |
| R4 | Detect the user's language and show available law in that language. | `docs/site.js` uses `navigator.languages` and routes the "preferred language" link to the matching language index. |
...
| R6 | Make the repository fully updatable using GitHub Actions CI/CD. | `.github/workflows/ci.yml` runs PR checks; `.github/workflows/refresh-laws.yml` discovers, fetches, builds, and deploys Pages. |
...
| R11 | Compile issue research under `docs/case-studies/issue-1`. | This case study plus `data/issue-1.json`, `data/js-template-file-tree.txt`, and `data/reference-urls.txt`. |
...
The PR includes a deterministic seed manifest with several major active Central Acts and a small section excerpt for one law so CI can verify the full build path without hitting government sites. The live refresh workflow is the production path for discovering the full active list and replacing seed excerpts with current official section text.

PDF-only regional sources are recorded and linked. Full conversion of scanned regional PDFs to Markdown should be added as a separate OCR/extraction stage after validating source quality and legal labeling.
...
- keep PR checks deterministic and quick;
