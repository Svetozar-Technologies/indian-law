...
| R6 | Make the repository fully updatable using GitHub Actions CI/CD. | `.github/workflows/ci.yml` runs PR checks plus a live official source smoke download; `.github/workflows/refresh-laws.yml` discovers, fetches all active laws on the default branch, builds, and deploys Pages. |
...
| R12 | Store own data/metadata in Links Notation instead of JSON. | Seed manifests, language config, generated catalog metadata, and issue metadata are readable `.lino` files using plain string references and `links-notation` validation instead of base64-encoded string data. |
